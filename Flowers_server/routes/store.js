...
route.get('/info', (req, res) => {
    ...
    storeList.forEach(({ courseID, storeID } = {}) => {
        let item = req.homelistDATA.find(item => parseFloat(item.id) === courseID);
...
route.get('/minus', (req, res) => {
    ...
    let ary = [];
    ary[0] = req.storeDATA.find(item => {
        return parseFloat(item.courseID) === parseFloat(courseID)
    });
    let c = req.storeDATA.findIndex(item => {
        return parseFloat(item.courseID) === parseFloat(ary[0].courseID)
    });
    req.storeDATA.splice(c, 1);
    writeFile(STORE_PATH, req.storeDATA).then(() => {
        res.send({ code: 0, msg: 'OK!' });
    }).catch(() => {
        res.send({ code: 1, msg: 'NO!' });
